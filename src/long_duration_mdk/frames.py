...
def create_frame(
    ...
    **kwargs,
    ...
    See Also
    --------
    long_duration_mdk.create_frame_from_record
    long_duration_mdk.expand_frame_per_record

    Examples
    --------
    >>> import pandas as pd
    >>> from long_duration_mdk import create_frame
    >>> frame = create_frame(
...
def create_frame_from_record(
    ...
    **kwargs,
    ...
    If wanting to do this for many records use the function ``long_duration_mdk.expand_frame_per_record``.
    ...
    col_date_nm : str
        The column name to assign the date column.
    kwargs :
        See kwargs under long_duration_mdk.create_frame
    ...
    See Also
    --------
    long_duration_mdk.create_frame
    long_duration_mdk.expand_frame_per_record

    Examples
    --------
    >>> import pandas as pd
    >>> from long_duration_mdk import create_frame_from_record
    >>> record = pd.DataFrame(
    >>>     {
    >>>         "POLICY": ["P1"],
    >>>         "GENDER": ["M"],
...
def expand_frame_per_record(
    ...
    **kwargs,
    ...
    This function expands the function ``long_duration_mdk.create_frame_from_record`` to cover the
    ...
    --------
    >>> import pandas as pd
    >>> from long_duration_mdk import expand_frame_per_record
    >>> df = pd.DataFrame(
    >>>     {
    >>>         "POLICY": ["P1", "P2"],
    >>>         "GENDER": ["M", "F"],
...
def frame_add_exposure(
    ...
    *,
    ...
    --------
    >>> import pandas as pd
    >>> from long_duration_mdk import frame_add_exposure
    >>>
    >>> frame = pd.DataFrame(
    >>>     {
...
def frame_add_weights(
    ...
    *,
    ...
    --------
    >>> import pandas as pd
    >>> from long_duration_mdk import frame_add_weights
    >>>
    >>> frame = pd.DataFrame(
    >>>     {
    >>>         "BEGIN_DURATION_COL": pd.to_datetime(
...
def frame_filter(
    ...
    *,
    ...
    --------
    >>> import pandas as pd
    >>> from long_duration_mdk import frame_filter
    >>>
    >>> frame = pd.DataFrame(
    >>>     {
    >>>         "BEGIN_DURATION_COL": pd.to_datetime(
